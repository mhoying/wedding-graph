...
export default function App() {
  ...
  const [nodes, setNodes] = useState(() => {
    try {
      const saved = localStorage.getItem('wedding_graph_nodes_v77');
      if (saved) {
  ...
  const [links, setLinks] = useState(() => {
    try {
      const saved = localStorage.getItem('wedding_graph_links_v77');
      if (saved) {
        const parsed = JSON.parse(saved);

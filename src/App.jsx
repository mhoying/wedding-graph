...
export default function App() {
  ...
  useEffect(() => {
    try {
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith('wedding_graph_nodes_') && key !== 'wedding_graph_nodes_v115') {
  ...
  const [nodes, setNodes] = useState(() => {
    try {
      const saved = localStorage.getItem('wedding_graph_nodes_v115');
      if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('wedding_graph_nodes_v115', JSON.stringify(nodes));
  }, [nodes]);

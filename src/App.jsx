...
export default function App() {
  ...
  useEffect(() => {
    try {
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith('wedding_graph_nodes_') && key !== 'wedding_graph_nodes_v116') {
  ...
  const [nodes, setNodes] = useState(() => {
    try {
      const saved = localStorage.getItem('wedding_graph_nodes_v116');
      if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('wedding_graph_nodes_v116', JSON.stringify(nodes));
  }, [nodes]);
